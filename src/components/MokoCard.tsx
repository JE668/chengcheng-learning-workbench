...
export default function MokoCard({
  ...
  const body = (
    <div className={`rounded-3xl p-4 shadow-lg hover:shadow-2xl transition transform hover:-translate-y-1 ${color} text-white overflow-hidden relative`}>
      {badge && <div className="absolute top-2 right-2 z-10 px-2 py-1 rounded-full bg-white/90 text-moko-violet text-xs font-black shadow">{badge}</div>}
      <Image src={img || '/moko/lemei.jpg'} alt={title} width={128} height={128} className="w-24 h-24 md:w-32 md:h-32 object-cover rounded-2xl border-4 border-white/40 shadow mb-3" />

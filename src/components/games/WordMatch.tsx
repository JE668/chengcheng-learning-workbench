...
export default function WordMatch({ onFinish }: { onFinish: (score: number) => void }) {
  const [pairs] = useState(() => [...words].sort(() => 0.5 - Math.random()).slice(0, 7));
  ...
  function click(i: number) {
    ...
    const next = [...cards]; next[i].flipped = true; setCards(next);
    ...
    if (nf.length === 2) {
      ...
      setTimeout(() => {
        const m = [...cards];
        if (match) { m[a].matched = true; m[b].matched = true; }
        m[a].flipped = false; m[b].flipped = false;
        setCards(m); setFlipped([]);
      }, match ? 400 : 800);
    }
  }
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl p-4 md:p-6">
      <div className="flex justify-between mb-4">
        ...
        <span className="font-bold text-moko-rose">已解锁 {cards.filter((c) => c.matched).length / 2}/7</span>
      </div>
        ...
        {cards.map((c, i) => (
          <button
            ...
            className={`aspect-square rounded-xl text-sm md:text-lg font-bold flex items-center justify-center transition ${
              c.matched ? 'bg-moko-mint text-white opacity-60' : c.flipped ? 'bg-moko-pink text-white' : 'bg-gradient-to-br from-moko-violet to-moko-purple text-white hover:scale-105'
            }`}
          >

...
export default function LetterAdventure({ onFinish }: { onFinish: (score: number) => void }) {
  const [mode] = useState<'mixed'>('mixed');
  ...
  function finish(c: number) { setDone(true); onFinish(c * 10 + Math.max(0, time)); }
  ...
  function pick(l: string) {
    ...
    else { setIdx(idx + 1); setRound(makeRound(mode)); }
  }
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl p-6 text-center">
      ...
      <div className="flex justify-center gap-2 md:gap-4 mb-8 text-3xl md:text-5xl font-black text-moko-violet">
        {round.seq.map((c, i) => (
          <div key={i} className={`w-14 h-14 md:w-20 md:h-20 rounded-xl flex items-center justify-center shadow ${i === round.hiddenAt ? 'bg-moko-pink text-white' : 'bg-yellow-100'}`}>
            {i === round.hiddenAt ? '?' : c}
          </div>
        ))}
      </div>
      <div className="flex justify-center gap-3 flex-wrap">
        {round.choices.map((c) => (
          <button key={c} onClick={() => pick(c)} className="w-16 h-16 md:w-20 md:h-20 text-3xl font-black rounded-2xl bg-gradient-to-r from-moko-yellow to-moko-gold text-white shadow hover:scale-110 transition">{c}</button>
        ))}
      </div>
    </div>
  );

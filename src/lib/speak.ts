 ...
 * - 朗读主路径：首句优先用浏览器原生 Web Speech **即时出声**（零网络往返，专治延迟高）；
 *   同时后台预热服务端 /api/tts 缓存。后续重复朗读或本机无对应语种嗓音（如 iPad 仅粤语）
 *   时，改走服务端 /api/tts（Edge 神经嗓音 zh-CN-XiaoxiaoNeural，普通话兜底、跨设备音质一致）。
 *   本地首句未真正出声（iPad Safari 等常见静音）→ 立即降级服务端，保证「一定有声音」。

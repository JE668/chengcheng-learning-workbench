...
/**
 * Web Speech API - 宽松模式
 * 接受任何 zh-* / en-* 嗓音（包括粤语、台式、英式）
 */
export class WebSpeechLooseEngine implements TTSEngine {
  readonly type: TTSEngineType = 'web-speech-loose';
  readonly name = 'Web Speech (Loose)';
  ...
  private getVoice(lang: TTSLanguage): SpeechSynthesisVoice | null {
    ...
    const voice = voices.find(v => (v.lang || '').toLowerCase().startsWith(targetPrefix)) ?? null;
  ...
  async speak(text: string, lang: TTSLanguage, options: TTSOptions = {}): Promise<TTSResult> {
    ...
    return new Promise((resolve) => {
      ...
      if (!voice) {
        resolve({ success: false, error: 'No loose voice available', engineUsed: this.type, latencyMs: performance.now() - startTime });
        return;
      }
      ...
      utterance.onend = () => {
        if (options.pauseMs && options.pauseMs > 0) {
          endTimer = setTimeout(() => finish(true), options.pauseMs);
        } else {
          finish(true);
        }
      };
      ...
      endTimer = setTimeout(() => finish(started, 'end timeout'), 15000);
    });
  }
